...
class VideoSignalingClient(context: Context, private val listener: VideoListener) {
    ...
    init {
        try {
            ...
            socket?.connect()
        } catch (e: Exception) { Log.e("VideoSig", "Error: ${e.message}") }
    }

    fun findBuddy(pref: String) { socket?.emit("find_buddy", JSONObject().put("mode", "video").put("prefGender", pref)) }
